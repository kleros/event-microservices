...
const createEventHandlers = (chainId) => {
  ...
  return {
    Draw: async (_, klerosLiquid, event) => {
      const dispute = await klerosLiquid.methods.disputes(event._disputeID).call();
              ...
              event._appeal === "0"
                ? `${timeAgo.format(
                    (Number(dispute.lastPeriodChange) +
                      Number((await klerosLiquid.methods.getSubcourt(dispute.subcourtID).call()).timesPerPeriod[0])) *
                      1000
                  )}`
      ...
      return [
        {
          ...
          dynamic_template_data: {
            endTime: `${timeAgo.format(
              (Number(dispute.lastPeriodChange) +
                Number((await klerosLiquid.methods.getSubcourt(dispute.subcourtID).call()).timesPerPeriod[2])) *
                1000
            )}`,
            caseNumber: event._disputeID,
            caseUrl: `https://court.kleros.io/cases/${event._disputeID}?${qs.stringify({ requiredChainId: chainId })}`,
...
const createLambdaHandler = (createWeb3, createContract) => {
  return async (_event, _context, callback) => {
    ...
    let notifications = [];
    try {
      notifications = await handlers[event.event](web3, createContract(web3), event);
    } catch (err) {
    ...
    for (const notification of notifications) {
      ...
      try {
        ...
        const pushNotifications = item && item.Item && item.Item.pushNotifications && item.Item.pushNotifications.BOOL;

        let pushNotificationsData;
        try {
          pushNotificationsData =
            item && item.Item && item.Item.pushNotificationsData ? JSON.parse(item.Item.pushNotificationsData.S) : null;
        } catch (err) {
...
module.exports.post = createLambdaHandler(
  _web3,
  (web3) => new web3.eth.Contract(_klerosLiquid.abi, process.env.KLEROS_LIQUID_ADDRESS_MAINNET)
);

module.exports.postXDai = createLambdaHandler(
  _web3.xdaiChain,
  (web3) => new web3.eth.Contract(_xKlerosLiquid.abi, process.env.X_KLEROS_LIQUID_ADDRESS_XDAI)
);
